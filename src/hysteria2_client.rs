...
struct Hysteria2Connection {
    ...
    /// The QUIC endpoint that owns this connection's driver task and buffers.
    /// Must be stored to prevent the endpoint from being dropped (which would
    /// leak its background driver task and internal buffers).
...
impl Hysteria2ConnectionManager {
    fn new(
        ...
        transport_config
            ...
            .receive_window((4u32 * 1024 * 1024).into())
            .stream_receive_window((1u32 * 1024 * 1024).into())
            .initial_mtu(1200)
            .min_mtu(1200)
            .mtu_discovery_config(Some(quinn::MtuDiscoveryConfig::default()))
            .enable_segmentation_offload(!cfg!(any(target_os = "android", target_os = "ios")))
    ...
    async fn get_connection_with_sessions(
        ...
        // Close old endpoint to free QUIC driver resources and buffers
        if let Some(old_conn) = guard.take() {
            let old_endpoint = old_conn.endpoint;
            tokio::spawn(async move {
                old_endpoint.close(0u32.into(), b"reconnecting");
                old_endpoint.wait_idle().await;
            });
        }

        let (connection, endpoint) = self.connect_and_authenticate().await?;
    ...
    ///
    /// Returns both the connection AND the endpoint. The endpoint must be kept
    /// alive to prevent its background driver task and buffers from leaking.
    async fn connect_and_authenticate(&self) -> std::io::Result<(quinn::Connection, quinn::Endpoint)> {
        let base_addr =
